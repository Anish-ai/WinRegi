...
Handles search queries and returns relevant Windows settings
"""
...
class SearchEngine:
    """AI-powered search engine for Windows settings"""
    ...
    def search(self, query: str) -> List[Dict[str, Any]]:
        ...
        results = self.db_manager.search_settings(query)
        
        # Enhance results with relevance scoring
        scored_results = self._score_results(results, processed_query)
        
        # Sort by relevance score
        sorted_results = sorted(scored_results, key=lambda x: x['relevance_score'], reverse=True)
        
        # Filter out low-relevance results
        if sorted_results and sorted_results[0]['relevance_score'] > 0.5:
            threshold = max(0.3, sorted_results[0]['relevance_score'] * 0.5)
            filtered_results = [r for r in sorted_results if r['relevance_score'] >= threshold]
        else:
    ...
    def get_setting_recommendations(self, query: str = None) -> List[Dict[str, Any]]:
        ...
        if query:
            ...
            # user history, popular settings, etc.
            ...
            for category in categories[:3]:  # Get settings from top 3 categories
                ...
                if category_settings:
                    recommendations.extend(category_settings[:2])  # Get top 2 settings from each category
